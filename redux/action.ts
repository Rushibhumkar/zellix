...
export const onLogOutEmpty = () => {
  return async (dispatch) => {
    try {
      ...
      console.log("onLogOutEmptyError", error);
    }
  };
};

export const refetchLoggedInUser = () => {
  return async (dispatch) => {
    try {
      const localUser = await getDataJson("userDetail");
      if (!localUser?._id) return;

      const apiUser = await userDetailHRM({ id: localUser._id });
      const mergedUser = { ...localUser, ...apiUser };

      dispatch(setUserInfo(mergedUser));
    } catch (e) {
      console.log("refetchLoggedInUser error", e);
    }
  };
};
