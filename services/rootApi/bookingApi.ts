...
export const updateBooking = async ({ id, data, onSuccess }) => {
  try {
    const res = await axiosInstance.post(
      `/api/booking/updateBookingById/${id}`,
      data,
      {
        headers: {
          "Content-Type": "multipart/form-data",
...
export const deleteBookings = (data: [string]) => {
  return axiosInstance
    .post("/api/booking/deleteBookingsById", data)
    .then((res) => res);
};

export const rejectBooking = (id: string, data: any) => {
  return axiosInstance
    .post(`/api/booking/bookingRejectedById/${id}`, data)
    .then((res) => res);
};

export const approvedBooking = (id: string, data: any) => {
  return axiosInstance
    .post(`/api/booking/bookingApprovedById/${id}`, data)
    .then((res) => res);
};

export const updateCase = (id: string, data: any) => {
  return axiosInstance
    .post(`/api/booking/caseFileUpload/${id}`, data, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    })
    ...
    .catch((err) => {
      myConsole("errUpdateCaser", err?.response?.data);
...
export const getBookingById = async (id) => {
  try {
    const response = await axiosInstance.get(
      `api/booking/getBookingDetailsById/${id}`
    );
    return response.data;
  } catch (err) {
