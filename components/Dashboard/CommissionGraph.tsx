...
const CommissionGraph = ({ isLoading, item }) => {
  ...
  // }, [])
  useEffect(() => {
    let temp = item?.weeklyCommission?.map((el) => {
      return {
  ...
  const handleSelect = (
    type: "weeklyCommission" | "monthlyCommission" | "yearlyCommission"
  ) => {
    if (type === "weeklyCommission") {
      ...
      let temp = item?.monthlyCommission?.map((el) => {
        return {
          value: el?.totalCommission / 100000,
          label: el?.month,
          frontColor: "#177AD5",
      ...
      let temp = item?.yearlyCommission?.map((el) => {
        return {
          value: el?.totalCommission / 100000,
          label: el?.year,
          frontColor: "#177AD5",
      ...
      {!isLoading ? (
        <View>
          ...
          {chart?.length > 0 ? (
            ...
            >
              <NoDataIcon width={240} height={240} />
            </View>
          )}
