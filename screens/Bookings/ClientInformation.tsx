...
const ClientInformation = () => {
  ...
  const [date, setDate] = useState(
    params?.initialValues?.createdAt || new Date()
  );
  ...
  const [developerInvoice, setDeveloperInvoice] = useState([]);
  const [agentCommissionRelease, setAgentCommissionRelease] = useState([]);
  ...
  useEffect(() => {
    if (params?.initialValues?.paymentProofArr?.length > 0) {
      setPaymentImgArr(
        params?.initialValues?.paymentProofArr?.map((el) => {
          return {};
        })
      );
    }
    if (params?.initialValues?.otherDocs?.length > 0) {
      setOtherDocs(
        params?.initialValues?.otherDocs?.map((el) => {
          return {};
        })
      );
    }
  ...
  const onSelectMultipleDocs = (img, setStateFn) => {
    setStateFn((prev) => [...prev, img]);
  };
  ...
  const handleFileError = (key, fileSize) => {
    setErrors((prevErrors) => ({
      ...prevErrors,
      [key]: isImageMoreThanTwoMb(fileSize),
    }));
  };
  useEffect(() => {
    for (const key in errors) {
      if (typeof errors?.[key] === "object" && errors?.[key]?.length > 0) {
        let a = errors?.[key]?.some(
          (el) => el === "File should be smaller than 2 MB"
        );
        if (a) {
              ...
              <>
                ...
                <>
                      ...
                      return (
                        <View key={i}>
                          <ExpoImagePicker
                            ...
                            boxContainerStyle={{ marginBottom: 15 }}
                            onSelect={(a) =>
                              onSelectMultiplePayment(a.assets[0], i, "payment")
                            }
                          />
                          {errors?.payment?.[i] && (
                            <CustomText style={styles.errorText}>
                  ...
                  <ExpoImagePicker
                    ...
                    isMultiplePick={true}
                    boxContainerStyle={{ marginBottom: 15 }}
                    onSelect={(a) => onSelectMultipleDocs(a.assets[0], setAml)}
                  />
                  ...
                  <ExpoImagePicker
                    ...
                    boxContainerStyle={{ marginBottom: 15 }}
                    onSelect={(a) =>
                      onSelectMultipleDocs(a.assets[0], setKycForm)
                    }
                  />
                  ...
                  <ExpoImagePicker
                    ...
                    boxContainerStyle={{ marginBottom: 15 }}
                    onSelect={(a) =>
                      onSelectMultipleDocs(a.assets[0], setDeveloperInvoice)
                    }
                  />

                  {/* ---- Agent Commission Release ---- */}
                  <ExpoImagePicker
                    label="Agent Commission Release"
                    isMultiplePick={true}
                    boxContainerStyle={{ marginBottom: 15 }}
                    onSelect={(a) =>
                      onSelectMultipleDocs(
                        a.assets[0],
                        setAgentCommissionRelease
                      )
                    }
                  />
                  ...
                  <ExpoImagePicker
                    ...
                    boxContainerStyle={{ marginBottom: 35 }}
                    onSelect={(a) =>
                      onSelectMultipleDocs(a.assets[0], setReferralForm)
                    }
                  />
                </>
                <MainTitle
