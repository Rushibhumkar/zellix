...
import { useGetAttendanceChart, useGetLeaveChart, useGetUserStatusChart } from '../../hooks/useGetQuerryHRM';
...
const CircularBarChart = ({
    ...
    //
    const { data: userCount } = useGetUserStatusChart({ isEnable: type === 'userChart' && true })
    const { data: leaveCount } = useGetLeaveChart({ isEnable: type === 'leavesChart' && true });
    const { data: attendanceCount } = useGetAttendanceChart({ isEnable: type === 'attendanceChart' && true })
    ...
    useEffect(() => {
        ...
        if (type === 'userChart') {
            ...
            let temp = dummyUser?.map((el) => {
                let val = typeof (userCount?.[el?.key]) === 'number' ? userCount?.[el?.key] : 0
                return { ...el, value: val }
            })
            setPieCirData(temp)
        }
        if (type === 'leavesChart') {
            let temp = dummyLeave?.map((el) => {
                let val = typeof (leaveCount?.[el?.key]) === 'number' ? leaveCount?.[el?.key] : 0
                return { ...el, value: val }
            })
            setPieCirData(temp)
        }
        if (type === 'attendanceChart') {
            let temp = dummyAttendance?.map((el) => {
                let val = typeof (attendanceCount?.[el?.key]) === 'number' ? attendanceCount?.[el?.key] : 0
                return { ...el, value: val }
            })
            setPieCirData(temp)
        }
